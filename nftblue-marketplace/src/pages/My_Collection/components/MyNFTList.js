import React, { useEffect, useState } from 'react'
import { NFTList } from '../Create_NFT/NFTModal'
import InfiniteScroll from 'react-infinite-scroll-component'
import CircularProgress from '@mui/material-next/CircularProgress';
import { useDispatch, useSelector } from 'react-redux';
import { UpdateListMarketplace } from '@/scripts/actions/UpdateListMarketplace';
import axios from 'axios';
import { NFTMarketplace } from '../Create_NFT/NFTModal';
import { ethers } from "ethers";
import { MarketplaceList } from '../Create_NFT/NFTModal';
import { Bounce, ToastContainer, toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
import { listAccountDetail } from '../Create_NFT/NFTModal';
import { UpdateTransactionList } from '@/scripts/actions/UpdateListMarketplace';

function MyNFTList({nftList , nftMarketplace, myNFT, signerAddress, address, listAccount}) {

    const defaultImg = "data:image/jpeg;base64,"
    const [isSell, setIsSell] = useState(false)
    const [ID, setID] = useState()
    const [name, setName] = useState('')
    const [price, setPrice] = useState(0)
    const dispatch = useDispatch()
    const dataMarket = useSelector((state) => state.nftMarketplaceReducer.NFTMarketplace)

    const handleClick = (name, id) => {
        setIsSell(!isSell)
        setName(name)
        setID(id)
    }

    const sellNFT = async() => {
        if(nftMarketplace !== undefined) {
            try {
                const cost = ethers.utils.parseUnits(price, 'ether')
                const listingFee = await nftMarketplace.getListingFee()
                await myNFT.approve(process.env.NEXT_PUBLIC_NFT_MARKETPLACE_ADDRESS, ID)
                const marketItemID = await nftMarketplace.createMarketItem(process.env.NEXT_PUBLIC_MY_NFT_ADDRESS, ID, cost , 0, { value: listingFee.toString() })
                await marketItemID.wait()
                getMarketList()
                getTransactionList()
                toast.success('🦄 It is successfull NFT Selling!', {
                    position: "top-right",
                    autoClose: 5000,
                    hideProgressBar: false,
                    closeOnClick: true,
                    pauseOnHover: true,
                    draggable: true,
                    progress: undefined,
                    theme: "dark",
                    transition: Bounce,
                  })

            } catch (e) {
                console.log(e)
            }
        }
        
    }

    async function getMarketList () {
        const tokenFetch = await MarketplaceList(nftMarketplace)
        dispatch(UpdateListMarketplace(tokenFetch))
    }

    async function getTransactionList () {
        const NFTList = await listAccountDetail(listAccount, myNFT)
        dispatch(UpdateTransactionList(NFTList))
    }

    
    console.log('nftMarketplace', dataMarket)
    useEffect(() => {
        getMarketList()
        getTransactionList()
        console.log('list', nftList)
    },[nftList])
    return (
    <>
          <InfiniteScroll
            dataLength={nftList.length}
             loader={<CircularProgress color="tertiary" />}
             className=' w-[100%] flex flex-wrap gap-[50px]'
            >
         {
                      nftList.map((nftList, id) => (
                          <div key={id} className=' w-[300px] gap-4 h-[450px] bg-[#1E1F28] rounded-lg justify-center items-center flex flex-col text-white shadow-inner shadow-indigo-500'>
                               <div className=' w-[90%] h-[65%]'>
                                     <img src={nftList.data.imgUrl} alt="" className='shadow-xl shadow-indigo-500/50 w-[100%] h-[100%] rounded-lg object-cover' />
                               </div>
                               <div className=' w-[90%] flex flex-col gap-2 '>
                                   <span className=' text-xl font-medium'>{nftList.data.name}</span>
                                   <div className=' flex justify-between w-[100%]'>
                                       {
                                          nftList.data.rareLevel === "Limited"
                                          && 
                                          <div className=' flex'>
                                            <span className=' p-1 bg-yellow-500 rounded-xl'>{nftList.data.rareLevel}</span>
                                          </div>
                                      }
                                      {
                                          nftList.data.rareLevel === "Rare"
                                          && <span className=' p-1 bg-violet-700 rounded-xl'>{nftList.data.rareLevel}</span>
                                      }
                                      {
                                          nftList.data.rareLevel === "Common"
                                          && <span className=' p-1 bg-gray-500 rounded-xl'>{nftList.data.rareLevel}</span>
                                      }
                                      {
                                          nftList.data.rareLevel === ""
                                          && <span className=' p-1 rounded-3xl'></span>
                                      }
                                      {
                                        isSell === true && name === nftList.data.name 
                                        ?
                                        <div className=' text-orange-300 flex w-[40%]'>
                                            <input onChange={(e) => setPrice(e.target.value)} className=' text-black w-[50%] rounded-md' type='number'/>
                                            <span className='font-semibold ml-2'>ETH</span>
                                            <svg height="25" preserveAspectRatio="xMidYMid" viewBox="0 0 256 417" width="25" xmlns="http://www.w3.org/2000/svg"><path d="m127.9611 0-2.795 9.5v275.668l2.795 2.79 127.962-75.638z" fill="#343434"/><path d="m127.962 0-127.962 212.32 127.962 75.639v-133.801z" fill="#8c8c8c"/><path d="m127.9611 312.1866-1.575 1.92v98.199l1.575 4.601 128.038-180.32z" fill="#3c3c3b"/><path d="m127.962 416.9052v-104.72l-127.962-75.6z" fill="#8c8c8c"/><path d="m127.9611 287.9577 127.96-75.637-127.96-58.162z" fill="#141414"/><path d="m.0009 212.3208 127.96 75.637v-133.799z" fill="#393939"/></svg>
                                        </div>
                                        :
                                        ''
                                      }
                                  </div>
                                  <div className=' flex justify-between nftLists-center w-[100%]'>
                                      <div className=' flex gap-2 justify-center items-center'>
                                          <img className=' rounded-full w-[30px]' src={defaultImg} alt="" />
                                          <span className='text-xs'>{signerAddress}</span>
                                      </div>
                                      {
                                        isSell === true && name === nftList.data.name
                                        ?
                                        <button onClick={() => sellNFT()} className=' p-3 bg-red-400 rounded-full'>Confirm Sell</button>
                                        :
                                        <button onClick={() => handleClick(nftList.data.name, nftList.id)} className=' p-3 bg-blue-600 rounded-full'>Sell NFT</button>
                                      }
                                  </div>
                              </div>
                          </div>
                  )
              )
         }
        </InfiniteScroll>
        <ToastContainer
            position="top-right"
            autoClose={5000}
            hideProgressBar={false}
            newestOnTop={false}
            closeOnClick
            rtl={false}
            pauseOnFocusLoss
            draggable
            pauseOnHover
            theme="dark"
            transition={Bounce}
            />
 </> 
  )
}

export default MyNFTList