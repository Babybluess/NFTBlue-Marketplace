...
function MarketList({NFTdata, nftMarketplace, address, balance}) {
    ...
    const cancelSelling = async(id) => {
        if(nftMarketplace !== undefined ) {
            try {
                const cancel = await nftMarketplace.cancelMarketItem(process.env.NEXT_PUBLIC_MY_NFT_ADDRESS, id)
                await cancel.wait()
                console.log('data', data)
                toast.success('🦄 It is successfull NFT Cancel Selling!', {
                    position: "top-right",
                    autoClose: 5000,
                            ...
                            <div className=' w-[90%] flex flex-col gap-2 '>
                                ...
                                <div className=' flex justify-between items-center w-[100%]'>
                                    <div className=' flex gap-2 justify-center items-center'>
                                        ...
                                        <span className=' text-sm'>{`${item.seller?.substring(0,8)}...${item.seller?.substring(35)}`}</span>
                                    </div>
                                    {
                                        item.seller == address 
                                        ?
                                        <button onClick={() => cancelSelling(item.id)} className=' p-3 bg-red-400 rounded-full'>Cancel</button>
                                        :
                                        <Link href={{ pathname:`/NFTCard/${item.name}`,
                                                    query: {
                                                        nftID: item.id,
                                                        nftSeller: item.seller,
                                                        nftName: item.data.name,
                                                        nftImage: item.data.imgUrl,
                                                        nftRareLevel: item.data.rareLevel,
                                                        nftType: item.data.type,
                                                        nftPrice: item.price,
                                                        balanceSigner: balance
                                                    }
                                        }} state={{nftData: item}} className=' p-3 bg-blue-600 rounded-full'>Place a bid</Link>
                                    }
                                </div>
                            </div>
                        </div>
